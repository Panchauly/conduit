...
- **Real bug #1, caught by CI, not by local testing:** the first push had the guard's `replace_one` replacement built as bare `to_document(guard)` — `ProjectionGuard` itself carries no `target`/`entity_key` fields (they're the caller's identity, not guard state), and `replace_one` overwrites the *entire* document with exactly what it's given. Every guard write was therefore silently dropping both fields, so the unique index saw every entity as the same `{target: null, entity_key: null}` row: the second guard write anywhere in the process hit `E11000 duplicate key`, and — because that failure happened inside the transaction — the whole transaction (including the already-applied target-document write) rolled back with it. Fixed by a `MongoBackend::guard_doc()` helper that inserts `target`/`entity_key` into the serialized guard before every `replace_one`, the same way `write()` already did for the target document's `_id`.
- **Real bug #2, same root cause of "never rehearsed against a live server":** `mongo_err()` unconditionally mapped every Mongo error to `WriteFailed`, checking for the `TransientTransactionError`/`UnknownTransactionCommitResult` labels only at the `commit_transaction()` step. `mongo_concurrent_writers` (run for real for the first time in CI) surfaced the actual common case: MongoDB's error code 112 (`WriteConflict`, "Write conflict during plan execution and yielding is disabled") is raised from the losing transaction's *individual write* — inside `write()`/`delete()`, not at commit — so it was hitting the non-retryable path and failing the whole attempt instead of retrying. Fixed by moving the label check into `mongo_err()` itself, so any Mongo error from anywhere in the transaction (a read, a write, or the commit) that carries a retryable label becomes `DocumentError::WriteConflict` uniformly.
- Both bugs were invisible to `cargo build`/`cargo clippy`/a compile-check of the test file, and only surfaced once CI's `test-mongo` job ran the suite against a real replica set — exactly why this environment's lack of Docker matters for this phase specifically.
